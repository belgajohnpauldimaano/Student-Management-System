...
class DiscountFeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax())
        {
            ...
            $DiscountFee = DiscountFee::where('status', 1)->where('category', 1)->where('disc_type', 'like', '%'.$request->search.'%')->paginate(10);
        ...
        $DiscountFee = DiscountFee::where('status', 1)->where('category', 1)->paginate(10);
    ...
    public function modal_data (Request $request) 
    {
        ...
        if ($request->id)
        {
            $DiscountFee = DiscountFee::where('id', $request->id)->where('category', 1)->first();
        }
