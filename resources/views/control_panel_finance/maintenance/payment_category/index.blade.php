...
@section ('content')
    <div class="card card-default">
        ...
        <div class="card-header">
            <div>
                <h6 class="box-title">Payment Category List</h6>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="js-data-container">
                        @include('control_panel_finance.maintenance.payment_category.partials.data_list')
                    </div>
                </div>
            </div>
        </div>
...
@section ('scripts')
    <script>
        ...
        function fetch_data () {
            ...
            loader_overlay();
            $.ajax({
                url : "{{ route('finance.maintenance.payment_category') }}",
                type : 'POST',
                data : formData,
                processData : false,
                contentType : false,
        ...
        $(function () {
            $('body').on('click', '#js-button-add, .js-btn_update', function (e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url : "{{ route('finance.maintenance.payment_category.modal_data') }}",
                    type : 'POST',
            ...
            $('body').on('submit', '#js-form_payment_category', function (e) {
                ...
                $.ajax({
                    url         : "{{ route('finance.maintenance.payment_category.save_data') }}",
                    type        : 'POST',
                    data        : formData,
                    processData : false,
                    contentType : false,
            ...
            $('body').on('click', '.js-btn_deactivate', function (e) {
                ...
                alertify.confirm('Confirmation', 'Are you sure you want to deactivate?', function(){  
                    $.ajax({
                        url         : "{{ route('finance.maintenance.payment_category.deactivate_data') }}",
                        type        : 'POST',
                        data        : { _token : '{{ csrf_token() }}', id : id },
                        success     : function (res) {
                            $('.help-block').html('');
            ...
            $('body').on('click', '.js-btn_toggle_current', function (e) {
                ...
                alertify.confirm('Confirmation', 'Are you sure you want to '+toggle_title+' ?', function(){  
                    $.ajax({
                        url         : "{{ route('finance.maintenance.payment_category.toggle_current_sy') }}",
                        type        : 'POST',
                        data        : { _token : '{{ csrf_token() }}', id : id },
                        success     : function (res) {
                            $('.help-block').html('');
