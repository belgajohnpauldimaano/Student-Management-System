    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="js-data-container">
                @include('control_panel_finance.maintenance.downpayment.partials.data_list')
            </div>
        </div>
    </div>
@endsection
...
@section ('scripts')
    <script>
        ...
        function fetch_data () {
            ...
            loader_overlay();
            $.ajax({
                url : "{{ route('finance.maintenance.downpayment') }}",
                type : 'POST',
                data : formData,
                processData : false,
                contentType : false,
        ...
        $(function () {
            $('body').on('click', '#js-button-add, .js-btn_update_sy', function (e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url : "{{ route('finance.maintenance.downpayment.modal_data') }}",
                    type : 'POST',
            ...
            $('body').on('submit', '#js-form_downpayment', function (e) {
                ...
                $.ajax({
                    url         : "{{ route('finance.maintenance.downpayment.save_data') }}",
                    type        : 'POST',
                    data        : formData,
                    processData : false,
                    contentType : false,
            ...
            $('body').on('click', '.js-btn_deactivate', function (e) {
                ...
                alertify.confirm('Confirmation', 'Are you sure you want to deactivate?', function(){  
                    $.ajax({
                        url         : "{{ route('finance.maintenance.downpayment.deactivate_data') }}",
                        type        : 'POST',
                        data        : { _token : '{{ csrf_token() }}', id : id },
                        success     : function (res) {
                            $('.help-block').html('');
