...
@section ('content')
    <div class="row">
        @include('control_panel_admission.user_profile.partials.data_list')
        @include('control_panel_admission.user_profile.partials.modal_data')
    </div>  
@endsection
...
@section ('scripts')
    ...
    <script>
        ...
        function fetch_data () {
            ...
            loader_overlay();
            $.ajax({
                url : "{{ route('admin.admission_information') }}",
                type : 'POST',
                data : formData,
                processData : false,
                contentType : false,
        ...
        $(function () {
            ...
            $('body').on('submit', '#form--change-password', function (e) {
                ...
                $.ajax({
                    url : "{{ route('admission.my_account.change_my_password') }}",
                    type : 'POST',
                    data : formData,
                    processData : false,
                    contentType : false,
            ...
            $('body').on('click', '.btn--update-profile', function (e) {
                e.preventDefault();
                $.ajax({
                    url : "{{ route('admission.my_account.fetch_profile') }}",
                    type : 'POST',
            ...
            $('body').on('submit', '#form--update-profile', function (e) {
                ...
                var formData = new FormData($(this)[0]);
                $.ajax({
                    url : "{{ route('admission.my_account.update_profile') }}",
                    type : 'POST',
                    data        : formData,
                    processData : false,
                    contentType : false,
            ...
            $('body').on('submit', '#js-form_subject_details', function (e) {
                ...
                var formData = new FormData($(this)[0]);
                $.ajax({
                    url         : "{{ route('admin.admission_information.save_data') }}",
                    type        : 'POST',
                    data        : formData,
                    processData : false,
                    contentType : false,
            ...
            function readURL(input) {
                ...
                if (input[0].files && input[0].files[0] && (ext == "gif" || ext == "png" || ext == "jpeg" || ext == "jpg")) {
                    ...
                    reader.onload = function (e) {
                        ...
                        $.ajax({
                            url : "{{ route('admission.my_account.change_my_photo') }}",
                            type : 'POST',
                            data : formData,
                            processData : false,
                            contentType : false,
