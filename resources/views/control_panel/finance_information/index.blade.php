            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="js-data-container">
                        @include('control_panel.finance_information.partials.data_list')
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $(function () {
            $('body').on('click', '#js-button-add, .js-btn_update_sy', function (e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url : "{{ route('admin.finance_information.modal_data') }}",
                    type : 'POST',
            ...
            $('body').on('click', '.js-btn_deactivate', function (e) {
                ...
                alertify.confirm('Confirmation', 'Are you sure you want to deactivate?', function(){  
                    $.ajax({
                        url         : "{{ route('admin.finance_information.deactivate_data') }}",
                        type        : 'POST',
                        data        : { _token : '{{ csrf_token() }}', id : id },
                        success     : function (res) {
                            $('.help-block').html('');
