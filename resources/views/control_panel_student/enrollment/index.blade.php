...
@section ('content')    
    <div class="row" id="back_method" style="display: none;">
        ...
        <div class="col-md-12">
            <button style="margin-top: -3em" class="btn-success btn btn-sm float-right">
...
disc_total = 0;
        ...
        $('.downpaymentBankSelected').click( function(){
            ...
            $.each(downpayment, function (index, value) {                
                ...
                document.getElementById('bank_pay_fee').value = downpayment_bank_fee; 
        ...
        $('.downpaymentgSelected').click( function(){
            ...
            $.each(downpayment, function (index, value) {                
                downpayment_g_fee = parseFloat(value.fee);
                 document.getElementById('gcash_pay_fee').value = downpayment_g_fee; 
            }); 
...
$('body').on('change','#payment_category', function(){
    ...
    $('body').on('click', '.label-receipt-img', function (e) {
        e.preventDefault()
        $('.input-receipt-img').click()
    })
    $('body').on('change', '.input-receipt-img', function(e) {
        const val = e.target.value.split('\\')
        if (val.length > 1) {
            $('.label-receipt-img').text(val[val.length - 1])
        } else {
            $('.label-receipt-img').text('Choose file')
        }
    })
